...
export const Navigation: React.FC = () => {
  ...
  const baseLinkClass =
    'flex items-center gap-2 px-3 sm:px-4 py-2.5 min-h-11 rounded-xl font-semibold transition-all duration-200 whitespace-nowrap';

  const standardLinkClass = (path: string, activeClass: string) =>
    `${baseLinkClass} ${isActive(path) ? activeClass : 'text-slate-700 hover:text-slate-900 hover:bg-white/75'}`;
  ...
  const assistantLinkClass = (mobile = false) =>
    `relative overflow-hidden ${mobile ? 'w-full justify-start' : ''} flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-2 min-h-10 rounded-xl font-semibold transition-all duration-300 whitespace-nowrap border ${
      isActive('/assistant')
        ...
        : 'text-violet-700 border-violet-200/80 bg-white/75 hover:bg-white hover:border-violet-300 hover:shadow-md hover:shadow-violet-200/40 hover:-translate-y-0.5'
    }`;
  ...
  return (
    <nav className="sticky top-0 z-50">
      <div className="container-padded max-w-7xl mx-auto">
        <div className="ios-surface mt-3 mb-3 rounded-[24px] px-3 sm:px-4 h-16 flex items-center justify-between gap-2 lg:gap-3">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-blue-300/40">
              WA
            </div>
            <span className="hidden sm:inline text-xl font-extrabold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent tracking-tight">WeAssist</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-3 shrink-0">
            {isAuthenticated ? (
              ...
              <Link
                to="/login"
                className="flex items-center gap-2 px-3 sm:px-4 py-2.5 min-h-11 btn-primary text-sm"
              >
                <LogIn size={18} />
          ...
          <div className="flex md:hidden items-center gap-2">
            {!isAuthenticated ? (
              ...
              <button
                onClick={handleLogout}
                className="inline-flex items-center justify-center min-h-10 px-3 rounded-xl text-slate-700 hover:bg-red-50 hover:text-red-700"
                title="Sign out"
              >
                <LogOut size={16} />
              </button>
            ...
            <button
              ...
              className="inline-flex items-center justify-center min-h-10 px-3 rounded-xl border border-slate-200/80 text-slate-700 hover:bg-white/75"
        ...
        {mobileMenuOpen && isAuthenticated && (
          <div className="md:hidden pb-3 animate-in fade-in duration-200">
            <div className="ios-surface rounded-2xl p-2 space-y-1">
