...
export const EventCard: React.FC<EventCardProps> = ({ event, onClick }) => {
  ...
  return (
    <div
      className="ios-surface rounded-[22px] border border-slate-200/70 overflow-hidden hover:shadow-2xl hover:border-blue-300/70 hover:translate-y-[-6px] transition-all duration-300 cursor-pointer h-full flex flex-col group"
    ...
    >
      ...
      <div className="flex-1 p-4 sm:p-5 flex flex-col">
        <h3 className="text-lg font-extrabold tracking-tight text-slate-900 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors">{event.title}</h3>
      ...
      {/* Footer */}
      <div className="pt-4 px-4 sm:px-5 pb-4 sm:pb-5 border-t border-slate-200/70 space-y-2.5">
        <div className="flex items-center gap-3 text-sm text-slate-700 font-medium">
          <Calendar size={16} className="flex-shrink-0 text-blue-600" />
          <span>{new Date(event.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
          {event.startTime && <span className="text-slate-500 ml-1 text-xs">{event.startTime}</span>}
        </div>
        <div className="flex items-center gap-3 text-sm text-slate-700 font-medium">
        ...
        {event.totalSlots && event.registrationStatus !== 'closed' && (
          <div className="flex items-center gap-3 text-sm text-slate-700 font-medium pt-1">
